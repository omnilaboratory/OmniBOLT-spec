...
2. data:
    ...
    * [`32*byte`:`funding_redeem_hex`]: funding redeem hex.   
...
2. data:
    ...
    * [`32*byte`:`rd_hex`]: The hex of revockable delivery(RD1a) transaction.
    * [`32*byte`:`fundee_pubKey`]: the omni address of fundee Bob.
    * [`32*byte`:`rsmc_signed_hex`]: rsmc signed by Bob.
...
2. data:
    ...
    * [`32*byte`:`commitment_tx_hash`]: C(n+1)a's commitment transaction hash, if Alice is the payer.
    ...
    * [`32*byte`:`rsmc_hex`]: the hex of RSMC transaction. From the hex, payment information can be extracted and verified by the counterparty.  
    * [`32*byte`:`to_counterparty_tx_hex`]: The hex of the transaction that pays the counterparty money.
